...
export class Layout{
    constructor(openNewRoute){
        ...
        document.getElementById("balance-button").addEventListener('click', this.changeBalance.bind(this));
    }
    ...
    async changeBalance(){
        ...
        this.saveBalanceButton.addEventListener("click", this.saveBalance.bind(this));
        this.cancelButton.addEventListener('click', ()=>{this.modalSheet.style.display = "none"});
    }

    async saveBalance(){
        let balanceValue = this.newBalanceValue.value;
        if(balanceValue && balanceValue >= 0){
            const result = await HttpUtils.request('/balance', "PUT", true, {newBalance: balanceValue});
            if(result || !result.error){
                this.balanceElement.innerText = result.response.balance + "$";
            ...
            alert("Баланс должен быть заполнен, укажите корректное значение баланса - равен или больше 0! ");
            return;
        }
    }
}